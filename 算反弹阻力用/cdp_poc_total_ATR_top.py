...
def _numba_compiled_volume_profile_math_inflow_v6_2(
    mins_high, mins_low, mins_math_net_vol, start_bin, bin_size, num_bins
):
    """【Numba内核A：数学流向专用版】"""
    ...
    for i in range(data_len):
        h = mins_high[i]
        l = mins_low[i]
        mv = mins_math_net_vol[i]

        if mv == 0.0 or np.isnan(h) or np.isnan(l) or np.isnan(mv):
            continue
        if l > h: h = l
        ...
        fractional_vol = mv / float(crossed_bins_count)
...
def run_intraday_single_grid_atr_math_net_inflow(df_today, atr_multiplier, base_atr):
    """【方法A修正常规体：无需L2，纯数学价格重心切片净流入外壳】"""
    try:
        # 1. 基础管道数据清洗（只需新浪财经标准的 5 个字段）
        df_clean = df_today.dropna(subset=["Open", "High", "Low", "Close", "Volume"]).copy()
        if df_clean.empty or len(df_clean) < 5:
            return None

        # 🚨【核心数学清洗】：无需 Level-2，通过日内分时K线形态强制分拆多空方向量能
        # 防止分时线横盘导致 High == Low 分母为 0
        h_arr = df_clean["High"].to_numpy()
        l_arr = df_clean["Low"].to_numpy()
        c_arr = df_clean["Close"].to_numpy()
        v_arr = df_clean["Volume"].to_numpy()
        
        # 计算价格重心分布因子：收盘价在K线实体和上下影线中的相对位置
        range_arr = h_arr - l_arr
        # 1. 先计算好多空价格净差分子
        numerator = (c_arr - l_arr) - (h_arr - c_arr)
        # 2. 利用 np.divide 的原生安全防御，凡是 range_arr == 0 的地方直接输出 0.0，不触发任何警告
        factor = np.divide(numerator, range_arr, out=np.zeros_like(range_arr), where=range_arr > 0.0)

        
        # 生成虚拟净流入量（可正可负）
        df_clean["Math_Net_Volume"] = v_arr * factor

        # 2. 网格分辨率空间自适应平滑
        bin_size = float(base_atr * atr_multiplier)
        if bin_size < 0.001: bin_size = 0.001
        ...
        # 4. 执行连续 C-Layout 序列化空间重组
        mins_high = np.ascontiguousarray(df_clean["High"].to_numpy())
        mins_low = np.ascontiguousarray(df_clean["Low"].to_numpy())
        mins_math_net_vol = np.ascontiguousarray(df_clean["Math_Net_Volume"].to_numpy())

        # 🚨【核心调用点】：调用兼容正负数学因子的内核
        counts = _numba_compiled_volume_profile_math_inflow_v6_2(
            mins_high, mins_low, mins_math_net_vol, start_bin, bin_size, num_bins
        )

        # 5. 执行直方图最高峰扫描（采用绝对值最大原则，寻找多空博弈最激烈的价格锚点）
        if counts.sum() == 0:
            return round(np.round((intraday_high + intraday_low) / 2.0 / 0.01) * 0.01, 2)

        max_idx = np.argmax(np.abs(counts))
        ...
        return round(float(np.round(poc_price / 0.01) * 0.01), 2)
    except Exception as e:
        print(f" 🚨 方法A数学变体外壳运行异常: {e}")
        return None
...
def execute_all_period_pipeline(ticker_symbol, df_all_data, target_dates_pool, df_period_days, base_price=0.0, end_date_str=None, base_atr=None):
    ...
    try:
        ...
        for key, atr_multiplier in ratios.items():
            if df_period_days == 1:
                poc_matrix[key] = run_intraday_single_grid_atr_math_net_inflow(df_window, atr_multiplier, base_atr)
            else:
